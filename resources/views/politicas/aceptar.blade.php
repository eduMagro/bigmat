<x-guest-layout>
    <div class="w-full" x-data="{ tab: 'privacidad' }">
        ...
        <div class="text-center mb-4">
            <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100">Hola, {{ $user->name }}</h2>
            <p class="text-xs text-gray-500 dark:text-gray-400">Acepta las politicas para continuar</p>
        </div>
        ...
        <form method="POST" action="{{ route('politicas.aceptar') }}" id="formPoliticas">
            ...
            <div class="flex border-b border-gray-200 dark:border-gray-700 mb-3">
                <button type="button" @click="tab = 'privacidad'"
                    :class="tab === 'privacidad' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-400 dark:text-gray-500'"
                    class="flex-1 py-2 text-xs font-medium border-b-2 focus:outline-none">
                    Privacidad
                </button>
                <button type="button" @click="tab = 'cookies'"
                    :class="tab === 'cookies' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-400 dark:text-gray-500'"
                    class="flex-1 py-2 text-xs font-medium border-b-2 focus:outline-none">
                    Cookies
                </button>
                <button type="button" @click="tab = 'terminos'"
                    :class="tab === 'terminos' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-400 dark:text-gray-500'"
                    class="flex-1 py-2 text-xs font-medium border-b-2 focus:outline-none">
                    Terminos
                </button>
            </div>
            ...
            <div class="space-y-2 mb-4">
                ...
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="acepta_cookies" value="1"
                        class="h-4 w-4 text-blue-600 rounded border-gray-300 dark:border-gray-600 focus:ring-blue-500 dark:bg-gray-700">
                    <span class="ml-2 text-xs text-gray-700 dark:text-gray-300">Acepto <strong>Cookies</strong></span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input type="checkbox" name="acepta_terminos" value="1"
                        class="h-4 w-4 text-blue-600 rounded border-gray-300 dark:border-gray-600 focus:ring-blue-500 dark:bg-gray-700">
                    <span class="ml-2 text-xs text-gray-700 dark:text-gray-300">Acepto <strong>Terminos</strong></span>
                </label>
            </div>
            ...
            <div class="flex items-center justify-between">
                <span class="text-xs text-gray-400 dark:text-gray-500"><span id="checkCount">0</span>/3</span>
                <div class="flex items-center gap-3">
                    <button type="button" onclick="document.getElementById('logoutForm').submit()"
                        class="text-xs text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">Salir</button>
